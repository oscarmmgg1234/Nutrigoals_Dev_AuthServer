...
module.exports = class DB{
    ...
    connect(){
        this.db.connect((err)=>{if(err !== null)throw(err)})
    }
    
    registerUser(JSONObject, callback){
        let database = this.db
        bcrypt.hash(JSONObject.password, password_hash_rounds, function(err, hash){
        database.query("INSERT INTO users (user_id, user_fullname, user_email, user_password, user_username) VALUES (?,?,?,?,?)"
        ,[createID(), JSONObject.fullname, JSONObject.email, hash, JSONObject.username]
    ...
    loginUser(JSONObject, callback){
        if(JSONObject.username.includes('@')){//email
            ...
            this.db.query("SELECT user_password, user_id, user_username  FROM nutrigoalsDB.users WHERE user_username = (?)", [JSONObject.username],(err, result)=>{
                if(err !== null){console.log(err);return (callback({valid: false}))}
                else if(result.length > 0){
